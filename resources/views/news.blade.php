...
<section>
    <div class="container py-5">
        <form action="{{route('news')}}" method="GET" class="row mb-4">
        ...
        <div class="d-flex my-3 align-items-center gap-2">
            ...
            <div class="d-flex align-items-center gap-2 overflow-auto tag-box">
                @forelse ($tags as $tag)
                <a href="{{route('news.category',$tag->slug)}}" class="p-1 px-3 bg-light cursor-pointer d-block text-dark rounded-4 text-nowrap text-lowercase text-decoration-none">{{$tag->name}}</a>
                @empty
        ...
        <div class="row mt-4">
            ...
            <div data-aos="fade-down" data-aos-delay="300" data-aos-duration="1000" class="col-6 col-lg-4 col-xl-3">
                <a href="{{route('news.show',$new->slug)}}" class="d-block mb-4 text-decoration-none text-dark">
                    <img src="{{Voyager::image($new->image)}}" alt="Image {{$new->title}} News" class="w-100 mb-3" style="aspect-ratio:16/9">
