    ...
    <div class="container py-5">
        ...
        <div class="row">
            <div class="col-12 mb-4">
                <a href="{{route('sparepart.category',$sparepart->category->slug)}}" class="text-decoration-none text-secondary fw-semibold d-flex flex-wrap align-items-center gap-3">
                    <span class="iconify" data-icon="pajamas:arrow-left"></span>
                    <span class="text-danger fw-semibold">Sparepart</span>/ <span class="text-danger fw-semibold">{{$sparepart->category->name}}</span>/ <span class="text-secondary fw-semibold">{{$sparepart->title}}</span>
                </a>
            </div>
            <div class="col-12 col-lg-9">
                <h1 class="text-capitalize">{{$sparepart->title}}</h1>
                <div class="d-flex align-items-center gap-2 text-warning my-4">
                    @if ($sparepart->rating != 0)
                        @php
                            $i = 1;
                        @endphp
                        @while ($i <= (int)$sparepart->rating)
                ...
                <div class="mb-4">
                    <div class="owl-carousel item-carousel owl-theme">
                        @if (isset($sparepart->images))
                            @foreach (json_decode($sparepart->images, true) as $image)
                            ...
                            @endforeach
                        @else
                        	<img data-aos="fade-down" data-aos-delay="0" data-aos-duration="1000" src="{{ Voyager::image($sparepart->thumbnail('cropped')) == "" ? Voyager::image($sparepart->thumbnail('cropped')) : Voyager::image($sparepart->image)}}" alt="Image {{$sparepart->title}} sparepart" class="rounded-4 d-block w-100 my-3" style="aspect-ratio:1/1; object-fit:cover">
                        @endif
                    </div>
            ...
            <div class="col-12 col-lg-3 p-4">
                ...
                <div class="mt-3 d-flex align-items-center gap-2 flex-wrap">
                    <a href="{{route('sparepart.category',$sparepart->category->slug)}}" class="p-1 px-3 bg-danger bg-opacity-10 cursor-pointer d-block text-danger rounded-4 text-nowrap text-lowercase text-decoration-none">{{$sparepart->category->name}}</a>
                </div>
                @if (isset($banner))
...
<section>
    <div class="container py-5">
                ...
                <a href="{{route('sparepart.show',$suggest->slug)}}" class="text-decoration-none d-block">
                    ...
                    <div class="text-dark fs-6 fw-semibold py-1">
                        ...
                        <div class="d-flex align-items-center gap-3 text-danger">
                            ...
                            <span class="text-lowercase">{{$suggest->category->name}}</span>
                        </div>
                    </div>
                </a>
            </div>
        ...
        <div class="d-flex align-items-center justify-content-center my-4">
            <a href="{{route('sparepart')}}" class="btn btn-outline-dark rounded-0 ">See More</a>
        </div>
    </div>
</section>

@include('partials.section',['code'=>'sparepart-show'])
